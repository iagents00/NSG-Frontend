  ...
  return (
    <header className="h-20 lg:h-24 bg-white/80 backdrop-blur-xl border-b border-slate-200/60 flex items-center justify-between px-4 sm:px-6 lg:px-10 sticky top-0 z-40 shrink-0 shadow-sm transition-all duration-300">
      <div className="flex items-center gap-4">
        <button
          className="lg:hidden p-2 -ml-2 text-navy-900 hover:bg-slate-100 rounded-lg transition cursor-pointer"
          onClick={toggleSidebar}
        >
          <Menu className="w-6 h-6" />
        </button>
        <div>
          <h2 className="font-display text-lg lg:text-2xl font-bold text-navy-900 truncate max-w-[180px] lg:max-w-none tracking-tight">
            {title}
          </h2>
          <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mt-0.5 hidden sm:block">
            {subtitle}
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 lg:gap-6">
        ...
        <div className="relative">
          <button
            ...
            className="w-10 h-10 lg:w-11 lg:h-11 rounded-full bg-white flex items-center justify-center text-slate-600 hover:bg-slate-50 hover:text-navy-900 transition relative border border-slate-200 shadow-sm hover:shadow-md cursor-pointer"
          >
          ...
          {showNotifs && (
            <div className="absolute right-0 top-14 w-80 bg-white/95 backdrop-blur-xl rounded-2xl shadow-sovereign border border-slate-100 p-4 animate-fade-in-up origin-top-right z-50">
              <div className="flex justify-between items-center mb-3 pb-2 border-b border-slate-100">
                ...
                <span className="text-[0.6rem] font-bold text-blue-600 cursor-pointer">
                  Marcar leídas
                </span>
              </div>
        ...
        <button
          onClick={toggleAI}
          className="group flex items-center justify-center gap-3 bg-navy-950 text-white pl-4 pr-6 py-2.5 rounded-full text-xs lg:text-sm font-bold shadow-2xl hover:bg-navy-900 transition-all transform hover:-translate-y-0.5 border border-navy-800 hover:shadow-glow relative overflow-hidden cursor-pointer"
        >
          <div className="w-5 h-5 relative shrink-0 atom-container">
            <div className="w-full h-full atom-breathe">
              <svg
                viewBox="0 0 100 100"
                className="w-full h-full text-blue-300 group-hover:text-white transition-colors"
              >
                <circle
                  cx="50"
                  cy="50"
